...
<table class="table-moderna">
    ...
    <tbody id="productTable">
        ...
        while ($row = mysqli_fetch_assoc($query)) {
            ...
            echo "<td>" . htmlspecialchars($row['prod_categoria']) . "</td>";
            echo "<td>" . htmlspecialchars($row['prod_preciocomp']) . "</td>";
            echo "<td>" . htmlspecialchars($row['prod_preciovent']) . "</td>";
            echo "<td>" . htmlspecialchars($row['prod_talla']) . "</td>";
            echo "<td>" . htmlspecialchars($row['prod_color']) . "</td>";
            echo "<td>" . htmlspecialchars($row['prod_stock']) . "</td>";
            echo "<td>" . htmlspecialchars($row['prov_codigo']) . "</td>";

            $imagen_path = '';
            $imagenes = glob('../assets_producto/' . $row['prod_codigo'] . '.*');

            if (count($imagenes) > 0) {
                $imagen_path = $imagenes[0];
            } else {
                $imagen_path = '../assets_producto/sin-imagen.png';
            }
            ...
            echo "<td>
            <a href='update_producto.php?prod_codigo={$row['prod_codigo']}' class='boton-editar'>
            ...
            <a href='delete_producto.php?prod_codigo={$row['prod_codigo']}' class='boton-eliminar'>
                <svg class='svg' viewBox='0 0 512 512'>
...
document.getElementById("searchInput").addEventListener("keyup", function () {
    ...
    rows.forEach(row => {
        let prod_codigo = row.cells[0].textContent.toLowerCase();
        let prod_nombre = row.cells[1].textContent.toLowerCase();

        if (prod_codigo.includes(filter) || prod_nombre.includes(filter)) {
            row.style.display = "";
        } else {
